...
- **Test:** `liveContext.test.tsx` incluye tests de `lastContentRef` que verifican que `contentScreen` se incluya al cambiar contenido y se omita al navegar diapositivas. El mock de `@ecclesia/queries` debe cubrir también `obsTextUpdate`/`requestObsText`, y `@/hooks/useBibleSchema` va mockeado (si no, `Api.query` indefinido rompe todo el archivo). `isRemoteMode` es configurable por test vía `isRemoteModeRef`.
- **Apagar la proyección es una orden explícita, no estado espejado (REGLA):** el toggle «En Vivo» pasa por `setShowLiveScreen` del contexto, que en `isRemoteMode` emite `liveSetShowLiveScreen { active }`; el host lo aplica tal cual, así que el remoto **sí** puede apagar. Lo que el host ignora es `showLiveScreen: false` dentro de `liveStateUpdate` (el espejo pasivo), porque ese camino no distingue una orden del operador de un eco del ciclo de vida del cliente y cerraba las pantallas en vivo del host al cerrarse la app cliente. Encender por espejo sigue permitido. Ver [`sockets/AGENTS.md`](../../../api/src/sockets/AGENTS.md).
- **Salida de texto para OBS:** calcula el texto plano + referencia bíblica del item en vivo con `extractOverlayText`/`extractOverlayReference` (`app/lib/presentationOverlayText.ts`) y los emite por socket (`obsTextUpdate { text, reference, contentType }`, donde `contentType = itemOnLive.type`) para las browser sources `/obs/subtitle/<slug>` (cada una filtra por tipo). Requiere `showLiveScreen` (sin proyección en vivo, overlay vacío = «no live, nada en ningún lado»). Vacío también con `TIMER`, si el slide es medio, o si el texto está oculto/negro/logo (F9/F10/F11); Escape lo limpia. Al (re)activarse la proyección se fuerza el reenvío (bypass del dedup) para refrescar páginas /obs con estado obsoleto. Escucha `requestObsText` (late join). Solo el host emite (no en `isRemoteMode`). Ver [`obs/agents.md`](../../../../api/src/controllers/obs/agents.md).
...
- `book_id` es el **id numérico** del libro, NO la abreviatura. La abreviatura vive en `book_short` y el nombre completo en `book`. Nunca usar `book_id` como texto visible: mostraría `58 12:1` en vez de `Heb 12:1`.
- El nombre visible se resuelve en `resolveBibleBookName(bibleSchema, bookId, mode)` (`app/hooks/useBibleSchema.utils.ts`): modo `'short'` → `book_short` con fallback a `book`; modo `'complete'` → `book`. `getShortNameById`/`getCompleteNameById` son wrappers de esa utilidad, consumidos por `BibleTextRender` según `biblePresentationSettings.description`.
