...
- En `PresentationRender` (live), los layers `MEDIA` de tipo video usan sincronización por `live-media-state` (`Api.socket.listen.liveMediaState`) para que controles externos (panel live) puedan reproducir/pausar/reiniciar también videos embebidos en diapositivas de presentación.
- En `PresentationRender` (live), `LiveSyncedLayerVideo` no usa `autoPlay` ni `play()` en montaje: espera exclusivamente comandos por `live-media-state` (`Api.socket.listen.liveMediaState`); el `loop` depende de `item.videoLoop` para respetar la configuración por diapositiva.
...
- En `MediaRender` live, el evento nativo `onPause` del `<video>` se usa como salvaguarda: si el navegador/WKWebView pausa el video sin que se haya recibido un comando de pausa (`liveMediaState` con `action: 'pause'`), el componente vuelve a llamar `play()`, salvo que el video haya llegado al final y no tenga `loop`.
- En `MediaRender` live, el `<video>` no auto-reproduce al montar; espera comandos por `live-media-state` (`Api.socket.listen.liveMediaState`) y mantiene `playsInline`, mientras `loop` se controla con `currentItem.videoLoop` para respetar la configuración persistida de la slide.
