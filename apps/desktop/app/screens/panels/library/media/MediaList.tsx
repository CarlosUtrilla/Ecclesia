...
import { Folder, Trash2, Edit, Copy, Scissors, ClipboardPaste } from 'lucide-react'
import {
  ContextMenu,
  ContextMenuContent,
  ContextMenuItem,
...
export function MediaList({
  ...
  return (
    <div className="w-full h-full min-h-full overflow-hidden" onClick={handleBackgroundClick}>
      {allItems.length === 0 ? (
        renderEmptyState()
      ) : (
            ...
            return (
              <ContextMenu key={isFolder ? `folder:${item}` : `file:${item.id}`}>
                <ContextMenuTrigger asChild>
                  <div
                    className={cn(
                      'flex items-center h-8 gap-2 px-2 py-1 group',
                  ...
                  >
                    <div className="flex items-center justify-center flex-shrink-0">
                      {isFolder ? (
                        ...
                        <img
                          className={cn('max-h-8 max-w-8 rounded transition-all duration-200', {
