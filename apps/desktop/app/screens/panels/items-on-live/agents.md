...
- Para evitar cortes de contenido en listas/grillas largas, los renders de `SONG` y la grilla `list/grid` usan el mismo patrón de scroll que `PRESENTATION`: contenedor `flex-1 min-h-0 overflow-auto` alrededor del contenido principal.
