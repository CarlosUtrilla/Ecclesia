...
- Las diapositivas de video creadas por Importar Canva se inicializan con `videoLiveBehavior: 'auto'` (inicio automático) como valor predeterminado del flujo de importación; otros flujos de media mantienen `manual` por defecto.
...
- En inserción bíblica theme-aware con indicador en borde (`upScreen`/`downScreen`), la fusión de bounds mantiene el ancho del bloque de texto del tema y solo expande en eje vertical para incluir el indicador; así no se fuerza ancho al `100%` cuando el tema tiene separación lateral.
...
- En `textCanvasItem`, los items bíblicos muestran un micro-control dentro del recuadro (anterior/siguiente) para previsualizar fragmento por fragmento durante la edición. El split reutiliza EXACTAMENTE la misma lógica que la biblioteca/LIVE: `resolveChunkParts` (exportado desde `@/lib/presentationSlides`) sobre `splitLongBibleVerse`/`splitBibleRangeIntoVerses`, con `resolveBibleChunkMaxLength` según la config global de biblia (`useDefaultBiblePresentationSettings`). Así un versículo largo también se parte en varios fragmentos (no solo verso por verso), con paridad visual respecto a la proyección.
- El contador de ese micro-control es relativo a los fragmentos generados (ej. `1/3`, `2/3`, `3/3`); el `title` indica el verso actual y el fragmento. Solo aparece cuando hay más de un fragmento.
- El fragmento previsualizado se conserva por item durante la sesión del editor (props `persistedStepIndex`/`onPersistStepIndex`, respaldados por `stepPreviewByItemRef` en `editorCanvas`); no se pierde al seleccionar otro item y volver.
- El cuerpo del texto renderizado en el micro-control usa el `content` del fragmento (sin el número de verso incrustado); el indicador de verso lo pinta `BibleTextRender` desde el objeto `verse`, igual que en LIVE.
...
- El diálogo `bibleTextPicker.tsx` ("Versículo bíblico") ya NO renderiza su propia lista de versículos: reutiliza el componente compartido `@/ui/bible/bibleChapterVerseList` (`BibleChapterVerseList`), el mismo que usa la biblioteca (`library/bible/viewVerses`). Así el troceo de versículos largos en fragmentos (`splitLongBibleVerse` + `resolveBibleChunkMaxLength` con la config de biblia de la DB) y la UI de las filas son de una sola fuente. El picker le pasa selección por rango (`rowClassName`/`onRowClick` con `shiftKey`); la biblioteca le pasa cronograma/live/dnd + menú contextual. Cambios en el split o en el render de filas se reflejan en ambos automáticamente.
