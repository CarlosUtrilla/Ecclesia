...
| `ai` | AiController | `getProviderConfig`, `getAvailableModels`, `saveProviderConfig`, `extractFromText`, `extractFromPdf` |
...
- `themes.exportThemeToZip(id)` genera `~/Downloads/<tema>.zip` con `theme.json` (datos de DB, incluye `backgroundBlur` para desenfoque de fondo media) y, cuando aplica, incluye el asset de fondo respetando el `filePath` original del media. Si el `textStyle.fontFamily` del tema corresponde a una fuente personalizada instalada, también incluye su archivo (`fonts/*.ttf|otf`) en el ZIP.
