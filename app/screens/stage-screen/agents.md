...
- `index.tsx`: consume eventos IPC de live (`liveScreen-update`, `liveScreen-update-theme`), carga `StageScreenConfig` por pantalla y aplica tema/layout/state.
- `index.tsx` también soporta modo preview (`isPreview`) para reutilizar el render de stage dentro de paneles (ej. lista de pantallas).
...
- Los widgets textuales `message`, `clock` y `liveTitle` usan `AnimatedText` directo (sin animación) para reducir overhead de render en stage y previews.
