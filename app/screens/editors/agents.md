...
- **Payload backend**: el backend entrega `lyrics` ya parseado (aunque en DB se persiste como JSON string en `Song.lyrics`), así el editor consume estructura lista sin parsear en renderer.
...
- Cuando el target activo es `Indicador` y está en `upScreen/downScreen`, el preview reutiliza la lógica de bounds del texto normal para moverlo y redimensionarlo horizontalmente; persiste `textStyle.verseWidthPercent` y `textStyle.verseTranslateX`.
- Los callbacks de actualización del indicador (`position`, `width`, `translateX`) en ThemeEditor se apoyan en refs del estado observado para mantener identidad estable durante drag continuo y evitar cortes por re-renders de React Hook Form.
- Durante resize del indicador, `width` y `translateX` se aplican sobre un merge atómico en `watchedTextStyleRef` antes de `setValue`, evitando que dos updates consecutivos se pisen y corten el redimensionado.
- El ThemeEditor expone `onBibleVerseHorizontalBoundsChange` para aplicar `verseWidthPercent` + `verseTranslateX` en una sola operación por frame de interacción, reduciendo churn y cortes durante resize continuo.
- Para mantener fluido el resize del indicador, esa actualización horizontal se batch-ea con `requestAnimationFrame` en el editor antes de `setValue`, evitando escrituras de formulario en cada `pointermove` crudo.
