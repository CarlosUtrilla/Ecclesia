...
- En `PresentationRender` (live), `LiveSyncedLayerVideo` no usa `autoPlay` ni `play()` en montaje: espera exclusivamente comandos por `live-media-state`; el `loop` depende de `item.videoLoop` para respetar la configuración por diapositiva.
