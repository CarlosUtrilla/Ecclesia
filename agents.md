...
- **Integración IA para cronogramas (MVP):** Multi-proveedor configurable (OpenAI/Anthropic), extracción de referencias bíblicas desde texto libre, PDF o Word (.docx via `mammoth`, .doc legacy via `word-extractor`), dialog modal con tabs (texto/PDF/Word), inserción directa al cronograma. Backend en `apps/api/src/controllers/ai/`, frontend en `AIScheduleDialog.tsx`. Configuración de API key en Ajustes.
...
- **Salida de vídeo NDI**: nuevo módulo `apps/desktop/electron/main/ndiManager/` que publica la proyección como fuente NDI en la red local (para OBS/vMix/ATEM). Ventana offscreen que carga `/live-screen/ndi` → evento `paint` (BGRA) → `sender.video()` del addon nativo `@stagetimerio/grandiose` (NDI SDK 6). Config persistida en `Setting` (`NDI_OUTPUT_CONFIG`), diálogo `NdiOutputDialog` en el menú «OBS». Solo vídeo (sin audio) por ahora. Ver [`ndiManager/agents.md`](apps/desktop/electron/main/ndiManager/agents.md).
- **Importación PPTX con render real**: nuevo módulo `apps/desktop/electron/main/pptxRenderer/` que rasteriza cada diapositiva a PNG con `@aiden0z/pptx-renderer` (Apache-2.0) en una ventana offscreen, y las mete por el mismo camino que el PDF: imágenes en `__pptx/` oculto, `Presentation` de una slide por diapositiva y `Media` PPTX con `presentationId`. Sustituye al antiguo `pptxConverter.ts`, que no renderizaba (solo sacaba `<a:t>` con regex) y dejaba las diapositivas casi en blanco. Se conserva el `.pptx` original para poder re-rasterizar. El rasterizador se inyecta a la capa de API con `setPptxRasterizer()` porque necesita `BrowserWindow`. Ver [`pptxRenderer/agents.md`](apps/desktop/electron/main/pptxRenderer/agents.md).
- **Importación PDF**: Backend completo — `importPdf` endpoint que convierte cada página del PDF a IMAGE via `pdfjs-dist` v3 + `@napi-rs/canvas`, almacena en `__pdf/` oculto, crea Presentation one-slide-per-page y un Media PDF con `presentationId`. **Frontend completo**: `MediaCard` muestra icono PDF + placeholder, drag-and-drop acepta `.pdf`, file picker filtro incluido. **Live redirect**: cuando un Media PDF/PPTX se envía a live, resuelve su Presentation vinculada y muestra las diapositivas; en `items-on-live` se controla con `RenderPresentationLiveController` según `isPresentationLikeMedia` (`liveRenderTarget.ts`).
...
| Modificar la salida de vídeo NDI | [`ndi`](apps/desktop/electron/main/ndiManager/agents.md) + [`electron`](apps/desktop/electron/agents.md) |
| Tocar la importación o el rasterizado de PPTX | [`pptxRenderer`](apps/desktop/electron/main/pptxRenderer/agents.md) + [`media`](apps/api/src/controllers/media/agents.md) |
